...
class Agent:
    ...
    def simulation_sequence(self):
        ep = []
        while not self.simulator.game_over():
            ep.append((self.simulator.state,self.simulator.check_reward()))
    ...
    def reward_to_go(self,ep,state):
        k = 0
        for x in range(0,len(ep)):
            if ep[x] == state:
                k = x
        value = 0
        i = k
    ...
    def MC_run(self, num_simulation, tester=False):
        ...
        for simulation in range(num_simulation):
            ...
            episode = self.simulation_sequence()
            for s in episode:
                self.S_MC[s[0]] += self.reward_to_go(episode,s[0])
                self.N_MC[s[0]] += 1
    ...
    def TD_run(self, num_simulation, tester=False):
        ...
        for simulation in range(num_simulation):
            ...
            while not (cur_state==None):
                next = self.make_one_transition(self.default_policy(cur_state))
                next_state = next[0]
                next_reward = next[1]
                if next_state == None:
                    self.TD_values[cur_state] += self.alpha(self.N_TD[cur_state]) * (
                                cur_reward + DISCOUNT * 0 - self.TD_values[cur_state])
                else:
                    self.TD_values[cur_state] += self.alpha(self.N_TD[cur_state]) * (
                                cur_reward + DISCOUNT * self.TD_values[next_state] -
                                self.TD_values[cur_state])
                self.N_TD[cur_state] += 1
    ...
    def Q_run(self, num_simulation, tester=False, epsilon=0.4):
        ...
        for simulation in range(num_simulation):
            ...
            cur_reward = self.simulator.check_reward()
            while not (cur_state==None):
                a = self.pick_action(cur_state,epsilon)
                next = self.make_one_transition(a)
                next_state = next[0]
                next_reward = next[1]
                if next_state == None:
                    self.Q_values[cur_state][a] += self.alpha(self.N_Q[cur_state][a]) * (cur_reward + DISCOUNT *0 - self.Q_values[cur_state][a])
                else:
                    self.Q_values[cur_state][a] += self.alpha(self.N_Q[cur_state][a])*(cur_reward+DISCOUNT*max(self.Q_values[next_state][0],self.Q_values[next_state][1])-self.Q_values[cur_state][a])
                self.N_Q[cur_state][a] += 1
                cur_state = next_state
                cur_reward = next_reward
